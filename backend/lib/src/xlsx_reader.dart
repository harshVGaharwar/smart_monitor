...
class XlsxException implements Exception {
  /// Creates an exception describing why the workbook could not be read.
  const XlsxException(this.message);

  /// Why the workbook was rejected, phrased for the person who uploaded it.
...
class XlsxReader {
  ...
  /// Reads the first populated worksheet of the workbook in [bytes] as rows
  /// of display text. Throws [XlsxException] if no sheet can be read.
  ...
  static String? _resolveTarget(Archive archive, String? target) {
    ...
    final clean = target.replaceAll(r'\', '/');
    final bare = clean
        .replaceFirst(RegExp('^/+'), '')
        .replaceAll(RegExp(r'^(\.\./)+'), '');

    for (final candidate in {clean, bare, 'xl/$bare'}) {
      final normalised = candidate.replaceFirst(RegExp('^/+'), '');
  ...
  static Set<int> _dateStyles(Archive archive) {
    ...
    for (final fmt in doc.findAllElements('numFmt')) {
      ...
      if (RegExp('[ymdhs]').hasMatch(stripped.toLowerCase())) {
  ...
  static String _fmtSerialDate(double serial, bool epoch1904) {
    final base = epoch1904 ? DateTime(1904) : DateTime(1899, 12, 30);
